...
- ~~**All pricing figures:**~~ REAL — verified against the locked price list (see `LOC-Pricing.md` in the `Desktop\LOC\` repo). Current as of 3 Aug 2026: Single £55 (early-bird made permanent — see below) / Double £70 (permanent) / Range £125 / Free-Standing £70 / Gas Hob from £15 / Induction £15 / Extractor from £15 / Microwave £15 / Combi £20 / AGA & BBQ contact-us. **⚠️ `LOC_EARLYBIRD_END` in `functions.php` is still set to `2026-08-10` and will auto-revert Single/Double Oven prices to £70/£90 on that date — this contradicts the "made permanent" decision and has NOT yet been fixed in code. Urgent, see Session Log.**
...
| August 2026 (extras pricing lowered site-wide) | **Gas Hob, Ceramic/Induction Hob, and Extractor Hood dropped £25→£15; Microwave £25→£15; Combi Microwave £30→£20.** Standing price change (not early-bird/time-limited). Updated `data-price` attributes and displayed price text in `page-reserve-step1.php`, `page-reserve-step2.php`, and `page-services.php`. Checked `functions.php`'s duration-lookup object (30/30/30/20 *minutes* per extra, used for calendar-slot length) and confirmed it's unrelated to price — coincidentally similar numbers, left untouched. `LOC-Pricing.md` (in `Desktop\LOC\`) updated to match. **Found while doing this: `LOC_EARLYBIRD_END` in `functions.php` is still `2026-08-10` and will silently revert Single/Double Oven prices back up to £70/£90 on that date, contradicting the "early-bird made permanent" decision already recorded in `LOC-Pricing.md` (dated 3 Aug 2026, flagged urgent there too, still not actioned). Needs fixing before 10 Aug — remove/disable the auto-revert so £55/£70 stays standing.** Commit `183085f`. |
