...
| June 2026 (Phase 4 — reservation submission, calendar write, emails) | **Full reservation submission flow implemented.** Installed WP Mail SMTP (free). Created `reservation-handler.php` as a WordPress AJAX action (`wp_ajax_nopriv_loc_reservation`) — reads POST data, validates, writes PROVISIONAL Google Calendar event via `loc_create_provisional_booking`, sends notification email to `hello@leicesterovencleaning.co.uk` and confirmation email to customer. Submit handler in `loc_step3_script` updated: snapshots all sessionStorage values at top of handler, POSTs to `/wp-admin/admin-ajax.php` via `URLSearchParams` (`application/x-www-form-urlencoded`), shows inline error on failure, proceeds to confirmation panel on success. Key bugs fixed during implementation: (1) direct `wp-load.php` path approach failed on Local — switched to WordPress AJAX pattern; (2) WordPress `wp_magic_quotes()` adds slashes to all `$_POST` values — `wp_unslash()` required before `json_decode` on appliances JSON string; (3) sessionStorage values must be snapshotted into local variables at the very top of the click handler before any async work. Calendar event description now shows appliances as multi-line block. `loc_create_provisional_booking` updated to accept `$callback_time` as 9th parameter. Email delivery confirmed working via `wp_mail()` — SiteGround SMTP rejects outbound from non-SiteGround IPs on local dev; emails will send correctly on production. Commit `6cb340a`. |
